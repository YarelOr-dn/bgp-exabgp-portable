...
EXABGP_TOOLS = [
            ...
            _tool("exabgp_onboard", "Onboard DUT VLAN on IL DNAAS", "Plan IL DNAAS global-BD AC + DUT neighbor/AFI. execute=true with confirm_commit=false is host dnos dry_run only. confirm_commit=true commits leaf then DUT. Never silent-fallback to g_mgmt_v999. Does not call dnos-config MCP.", {"vlan": _int("Global VLAN for this peering"), "vlan_range": _text("Allocated VLAN range e.g. 2100-2199"), "device": _text("DUT hostname"), "bd_name": _text("Confirmed DNAAS global BD name"), "dnaas_leaf": _text("DNAAS leaf"), "bundle": _text("Leaf/DUT bundle name"), "dut_bundle": _text("DUT bundle if different"), "dut_ip": _text("DUT inband IP"), "gateway": _text("Inband gateway"), "subnet": _text("Prefix length or CIDR", "24"), "neighbor": _text("ExaBGP OOB neighbor IP"), "asn": _text("DUT BGP ASN"), "peer_as": _text("ExaBGP AS"), "selected_afis": _text("Comma-separated DUT neighbor AFI names"), "bd_show_text": _text("Optional injected BD show text (tests)"), "owner": _text("Lease owner"), "confirm_commit": _bool("Commit after dry_run (requires execute=true)", False), "execute": EXEC_ARG, "timeout_sec": _int("Timeout", 120)}, ["vlan"]),
